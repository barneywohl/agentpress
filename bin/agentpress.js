...
if (vcheck.error) {
  console.error(`agentpress: Python not found (tried '${py}'). Install Python >= 3.10 or set PYTHON= env var.`);
  process.exit(1);
}
...
if (!vm || parseInt(vm[1], 10) < 3 || (parseInt(vm[1], 10) === 3 && parseInt(vm[2], 10) < 10)) {
  console.error(`agentpress: Python >= 3.10 required, found: ${vstr}. Install a newer Python or set PYTHON= env var.`);
  process.exit(1);
}
